	...
	if(isset($_POST["sub"])){
		...
		if($ok){
			...
			if($ok){
				...
				$reqJour = "SELECT jour FROM tdf_etape WHERE jour = :jour AND n_epreuve != :n_epreuve";
				$params = array(":jour" => $jour, ":n_epreuve" => $n_epreuve);
				...
				if($ok){
					...
					$update = "UPDATE tdf_etape SET annee = :annee, ville_d = :ville_d, ville_a = :ville_a, code_cio_d = :code_cio_d, code_cio_a = :code_cio_a, distance = :distance, moyenne = :moyenne, cat_code = :cat_code, jour = to_date(:jour, 'DD/MM/YY') WHERE n_epreuve = :n_epreuve";
